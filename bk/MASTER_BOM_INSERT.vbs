...
Dim StdOut : Set StdOut = CreateObject("Scripting.FileSystemObject").GetStandardStream(2) 
    path = "G:\vbs_develop"
	filename = "MASTER_BOM_INSERT"
	namelog = filename & "_Log"
Set fso = CreateObject("Scripting.FileSystemObject")
Set file = fso.OpenTextFile( path & "\work\BOM_MASTER.sql", 1)
...
ej.open
ej.Execute("TRUNCATE BOM_MASTER")
	Set sql_sv = cn.Execute(content)

	Set count_sql = cn.Execute( " SELECT COUNT(*) CC FROM ( SELECT COUNT(*) ITEM FROM M_PLANT_PS MP GROUP BY MP.PARENT_ITEM_CD )  " )

	Set file = fso.OpenTextFile( path & "\work\BOM_MASTER_INS.sql", 1 )
	content = file.ReadAll
