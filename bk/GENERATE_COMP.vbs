...
dim ej, cn, path, log, filename, namelog, StdOut, Ng, Ref, Ind, GH
	...
	Set file = fso.OpenTextFile( path & "\work\" & "BOM_COMP_DEP.sql", 1)
	'Set file = fso.OpenTextFile( path & "\work\" & "BOM_COMP.sql", 1)
	content = file.ReadAll 
	
	Str_comp = content '& "'" & "10297584" & "'"
	Str_par = "SELECT ITEM_MASTER FROM BOM_MASTER LIMIT 20"'WHERE ITEM_MASTER IN ( '898320-8638-SP', '898232-6241-EP' ) "
	Rcs_comp = content
...
ej.Execute("TRUNCATE BOM_COMP_DEP")
...
Set file = fso.OpenTextFile( path & "\work\BOM_COMP_DEP_INS.sql", 1)
'Set file = fso.OpenTextFile( path & "\work\BOM_COMP_INS.sql", 1)
content = file.ReadAll
options = content
	GH = Cnt_master
	'STEP_PRO = 2
		...
		'GH = CInt(sql_c1("MC").value)				
			   Do Until sql_v1.eof
				options = ins
					...
					Next				
						options = Mid(options,1,Len(options)-1)
						options = options & "," & Put_Sq(Ref) & "," & lvl & " );"
						call Write_File(options, path & "\log\" & namelog & ".sql")
